  ...
  try {
    ...
    if (count[0].total === 0) {
      ...
      const [classes] = await db.query(
        "SELECT id, nom FROM classes ORDER BY id",
      );
      ...
      const uesForDB = coursesData.map(
        ([code, intitule, className, semestre]) => {
          const classeId = classMap[className] || 1;
          return [code, intitule, classeId, semestre];
        },
      );

      for (const ue of uesForDB) {
        await db.query(
...
export const createUE = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: "UE créée", id: rows[0].id });
  } catch (err) {
    console.error(err);
